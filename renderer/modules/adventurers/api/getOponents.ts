...
async function getOponent(adventurer: AdventurerType, maxDownside = 20, maxUpside = 0): Promise<AdventurerType[]> {
    ...
    const min_intelligence = adventurer.intelligence - maxDownside <= 0 ? 0 : adventurer.intelligence - maxDownside;
    ...
    const min_constitution = adventurer.constitution - maxDownside <= 0 ? 0 : adventurer.constitution - maxDownside;
    ...
    return advs.filter((enemy) => enemy.tokenId !== adventurer.tokenId).sort((a, b) => {
...
export async function getOponents(adventurers: AdventurerType[], maxDownside = 20, maxUpside = 0): Promise<{ [key: number]: AdventurerType }> {
    const map: { [key: number]: AdventurerType } = {}
    const used: number[] = []
    await Promise.all(adventurers.map(async (adv) => {
